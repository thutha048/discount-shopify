...
const ProductList = ["soap","dress", "t-shirt", "laptop"];

const Collections = ["soap","clothes","electronics","others"];
...
export default function PageName() {
  ...
  const [selected, setSelected] = useState(["COLLECTIONS"]); //react hooks useState(), hàm setSelected sẽ set state cho selected
  ...
  const handleSearchSecond = e =>{
    if(e.target.value === ""){
      ...
      const FilterProductList =Collections.filter(
        item => item.toLowerCase().includes(e.target.value.toLowerCase())
      );
  ...
  const {
    ...
    onSubmit: async form =>{
            ...
            if (form.discountMethod === DiscountMethod.Automatic) {
              response = await authenticatedFetch("/api/discounts/automatic",{ //discounts?
                method: "POST",
